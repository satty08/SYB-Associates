...
__export(src_exports, {
  ...
  lovableAssetsProxyPlugin: () => lovableAssetsProxyPlugin
});
module.exports = __toCommonJS(src_exports);
var import_lovable_tagger = require("lovable-tagger");
var import_vite = require("vite");

// src/assetsProxyPlugin.ts
var import_node_https = __toESM(require("https"), 1);
var ASSET_RE = /^\/__l5e\/assets-v1\//;
var PREVIEW_HOST_ENV = "LOVABLE_PREVIEW_HOST";
    ...
    configureServer(server) {
      ...
      const middleware = (req, res, next) => {
        ...
        if (!ASSET_RE.test(url))
          return next();
        const upstream = import_node_https.default.request(
          {
            host: previewHost,
            port: 443,
            method: req.method,
            path: url,
            headers: { ...pickRequestHeaders(req.headers), host: previewHost }
          },
          (upRes) => {
        ...
        upstream.on("error", (err) => {
          server.config.logger.warn(
            `[@lovable.dev/vite-tanstack-config] asset proxy ${url} \u2192 ${previewHost} failed: ${err.message}`
          );
          if (!res.headersSent) {
            res.statusCode = 502;
            res.end();
...
var TOOL_MANAGED_WATCH_IGNORES = [
  "**/.workspace/**",
  "**/.agents/**",
  "**/.claude/**",
  "**/.lovable/**",
...
var HANDLER_RAN_KEY = "__LOVABLE_TANSTACK_HANDLER_RAN__";
...
var BUILD_DIAGNOSTICS_PLUGIN_NAME = "lovable-build-error-diagnostics";
var MAX_DIAGNOSTIC_JSON_LENGTH = 6e3;
var MAX_DIAGNOSTIC_STRING_LENGTH = 2e3;
var MAX_DIAGNOSTIC_NESTED_ERRORS = 20;
var MAX_DIAGNOSTIC_DEPTH = 4;
var COMPACT_DIAGNOSTIC_STRING_LENGTH = 300;
var COMPACT_DIAGNOSTIC_NESTED_ERRORS = 3;
var COMPACT_DIAGNOSTIC_DEPTH = 3;
var DIAGNOSTIC_ERROR_FIELDS = [
  "name",
  "message",
  "plugin",
  "pluginCode",
...
function serializeOwnDiagnosticProperties(value, depth, seen) {
  ...
  if (keys.length > MAX_DIAGNOSTIC_NESTED_ERRORS) {
    output.properties_truncated = keys.length - MAX_DIAGNOSTIC_NESTED_ERRORS;
  }
  return output;
}
...
function serializeDiagnosticValue(value, depth, seen) {
  ...
  if (typeof value !== "object")
    return describeDiagnosticPrimitive(value);
  if (seen.has(value))
    return "[circular]";
  ...
  try {
    if (Array.isArray(value)) {
      return value.slice(0, MAX_DIAGNOSTIC_NESTED_ERRORS).map((item) => serializeDiagnosticValue(item, depth + 1, seen));
    }
    const record = value;
    const output = {};
    ...
    if (Array.isArray(nestedErrors)) {
      ...
      if (nestedErrors.length > MAX_DIAGNOSTIC_NESTED_ERRORS) {
        output.errors_truncated = nestedErrors.length - MAX_DIAGNOSTIC_NESTED_ERRORS;
      }
    } else if (nestedErrors != null && nestedErrors !== "") {
      output.errors = serializeDiagnosticValue(nestedErrors, depth + 1, seen);
    }
...
function compactDiagnosticValue(value, depth) {
  if (value == null)
    return value;
  if (typeof value === "string")
    return truncateDiagnosticStringTo(value, COMPACT_DIAGNOSTIC_STRING_LENGTH);
  if (typeof value === "number" || typeof value === "boolean")
    return value;
  if (typeof value !== "object")
    return describeDiagnosticPrimitive(value);
  if (depth >= COMPACT_DIAGNOSTIC_DEPTH)
    return "[max depth]";
  try {
    if (Array.isArray(value)) {
      const items = value.slice(0, COMPACT_DIAGNOSTIC_NESTED_ERRORS).map((item) => compactDiagnosticValue(item, depth + 1));
      if (value.length > COMPACT_DIAGNOSTIC_NESTED_ERRORS) {
        items.push(`[${value.length - COMPACT_DIAGNOSTIC_NESTED_ERRORS} items truncated]`);
      }
      return items;
    }
    const record = value;
...
function stringifyDiagnosticPayload(diagnostics) {
  ...
  if (full && full.length <= MAX_DIAGNOSTIC_JSON_LENGTH)
    return full;
  const compact = compactDiagnosticValue(diagnostics, 0);
  const compactPayload = compact != null && typeof compact === "object" && !Array.isArray(compact) ? { ...compact, diagnostics_truncated: true } : { diagnostics_truncated: true, value: compact };
  const compactJson = JSON.stringify(compactPayload);
  if (compactJson.length <= MAX_DIAGNOSTIC_JSON_LENGTH)
    return compactJson;
  ...
  return {
    ...
    buildEnd(error) {
      if (!error)
        return;
      try {
        const diagnostics = serializeDiagnosticValue(error, 0, /* @__PURE__ */ new WeakSet());
        const output = `
[lovable-build-error-diagnostics]
${stringifyDiagnosticPayload(diagnostics)}
[/lovable-build-error-diagnostics]
`;
        let written = false;
        const write = () => {
...
function defineConfig(configOrOptions = {}) {
  return async (env) => {
    ...
    if (command === "build" && isSandbox) {
      internalPlugins.push(lovableBuildErrorDiagnostics());
    }
    ...
    const shouldRunNitro = options.nitro !== false && command === "build";
    if (shouldRunNitro) {
      let nitroMod;
      try {
      ...
      if (nitroMod) {
        ...
        } else if (!nitroOpts.preset && !await nitroSupportsDefaultPreset()) {
          logWarning(
            'The installed `nitro` predates the `defaultPreset` option (3.0.260603-beta), so the Cloudflare fallback is ignored and a zero-config build may target Node instead. Upgrade nitro to >=3.0.260603-beta, or set an explicit `nitro: { preset: "cloudflare-module" }`.'
          );
        }
    ...
    if (command === "serve") {
      internalPlugins.push(lovableAssetsProxyPlugin());
    }
    if (mode === "development") {
    ...
    if (command === "serve" && mode === "development") {
      internalPlugins.push((0, import_lovable_tagger.componentTagger)({ jsxSource: true }));
    }
    let envDefine = {};
    ...
    if (isSandbox) {
      ...
      config = cleanServerConfig(
        (0, import_vite.mergeConfig)(config, {
          server: { host: "::", port: 8080, strictPort: true, watch: { ignored: [...TOOL_MANAGED_WATCH_IGNORES] } }
        })
      );
    } else {

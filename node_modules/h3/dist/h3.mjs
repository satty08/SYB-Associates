...
function handleCacheHeaders(event, opts) {
	...
	if (ifNoneMatch) cacheMatched = !!opts.etag && matchETag(ifNoneMatch, opts.etag);
...
async function serveStatic(event, options) {
	...
	let cacheMatched = false;
	const ifNoneMatch = event.req.headers.get("if-none-match");
	if (ifNoneMatch) cacheMatched = !!meta.etag && matchETag(ifNoneMatch, meta.etag);
	else if (mtimeDate) {
		const ifModifiedSinceH = event.req.headers.get("if-modified-since");
		cacheMatched = !!ifModifiedSinceH && new Date(ifModifiedSinceH) >= mtimeDate;
	}
	if (cacheMatched) return new HTTPResponse(null, {
		status: 304,
